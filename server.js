...
wss.on('connection', (client) => {
  ...
  // MEXC -> relay -> client
  // QUAN TRỌNG: thư viện 'ws' luôn đưa dữ liệu nhận được vào dạng Buffer (kể cả khi MEXC gửi TEXT
  // frame thuần JSON) — nếu forward thẳng bằng client.send(data) mà KHÔNG chỉ rõ { binary:false },
  // ws sẽ gửi tiếp dưới dạng BINARY frame. Phía trình duyệt nhận binary frame thì ev.data là 1
  // Blob (không phải string) -> JSON.parse(ev.data) ở frontend lỗi và bị catch NUỐT ÂM THẦM, không
  // log gì cả -> nhìn như "kết nối sống nhưng không nhận được tin nhắn nào". Phải truyền lại đúng
  // cờ isBinary mà upstream báo, để giữ nguyên loại frame (text/binary) khi relay qua.
  ...
  // client -> relay -> MEXC (cùng lý do isBinary như trên, áp dụng chiều ngược lại)
